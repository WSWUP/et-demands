...
def main(ini_path, time_filter, start_doy, end_doy, year_filter=''):
    ...
    var_list = ['ETact', 'NIWR', 'P_rz', 'P_eft', 'PrzF', 'PeftF', 'PPT']
        ...
        for station in unique_stations:
            ...
            if year_list:
                if time_filter == 'water_year':
                    ...
                    daily_df = daily_df[
                        (daily_df['WY_+1mo'] >= min(year_list)) &
                        (daily_df['WY_+1mo'] <= max(year_list))]
                else:
                    daily_df = daily_df[
            ...
            if time_filter == 'water_year':
                ...
                yearlygroup_df = daily_df.groupby('WY_+1mo', as_index=True).agg(a)
            else:
    ...
    if min_year == max_year:
        final_df = cells[['CELL_ID', 'CWETact_mn', 'CWNIWR_mn',
                          'CWPrzF_mn', 'CWPeftF_mn',
                          'CWPPT_mn', 'CWP_rz_mn','CWP_eft_mn']]
        ...
        final_df = cells[['CELL_ID', 'CWETact_mn', 'CWNIWR_mn', 'CWETact_md',
                      ...
                      'CWPPT_md', 'CWP_rz_mn', 'CWP_rz_md', 'CWP_eft_mn', 'CWP_eft_md']]
    ...
    # output folder
    output_folder_path = os.path.join(output_ws, 'OPENET_cropweighted_{}to{}'.format(
        min_year, max_year))
    if min_year == max_year:
        output_folder_path = os.path.join(output_ws, 'OPENET_cropweighted_{}'.format(
            min_year))
...
def arg_parse():
    ...
    parser.add_argument(
        '-t', '--time_filter', default='annual', choices=['annual',
                                                          'growing_season',
                                                          'doy', 'water_year', 'water_year_+1mo'], type=str,
